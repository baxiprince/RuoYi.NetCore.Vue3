...
public sealed class UnitOfWorkAttribute : Attribute, IAsyncActionFilter, IAsyncPageFilter, IOrderedFilter
{
  ...
  public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
  {
    ...
    // 创建分布式环境事务
    var (transactionScope, logger) = CreateTransactionScope(context);
  ...
  /// <param name="context"></param>
  /// <returns></returns>
  private (TransactionScope? transactionScope, ILogger<UnitOfWork> logger) CreateTransactionScope(FilterContext context)
  {
    // 是否启用分布式环境事务
    var transactionScope = UseAmbientTransaction
      ? new TransactionScope(TransactionScope,
        new TransactionOptions
        {
          ...
          Timeout = TransactionTimeout > 0 ? TimeSpan.FromSeconds(TransactionTimeout) : default
        }
  ...
  private static void BeginTransaction(FilterContext context, MethodInfo method, out IUnitOfWork _unitOfWork,
    out UnitOfWorkAttribute unitOfWorkAttribute)
  {
    // 解析工作单元服务
  ...
  private static void CommitTransaction(FilterContext context, IUnitOfWork _unitOfWork, UnitOfWorkAttribute unitOfWorkAttribute,
    FilterContext resultContext)
  {
    // 获取动态结果上下文
