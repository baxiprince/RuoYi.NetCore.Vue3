...
public class MemoryCache : ICache
{
  ...
  {
    return await GetCacheAsync(key);
  }
  ...
  {
    await SetCacheAsync(key, value, minutes);
  }
  ...
  {
    SetCache(key, JSON.Serialize(value!));
  }

  public void Set<T>(string key, T value, long minutes)
  {
    SetCache(key, JSON.Serialize(value!), minutes);
  }
  ...
  public async Task<T> GetAsync<T>(string key)
  {
    try
    {
      var valueString = await GetCacheAsync(key);
      if (string.IsNullOrEmpty(valueString)) return default!;
  ...
  {
    await SetCacheAsync(key, JSON.Serialize(value!));
  }

  public async Task SetAsync<T>(string key, T value, long minutes)
  {
    await SetCacheAsync(key, JSON.Serialize(value!), minutes);
  }
