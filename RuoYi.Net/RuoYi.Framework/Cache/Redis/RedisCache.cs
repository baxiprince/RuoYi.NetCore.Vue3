...
public class RedisCache : ICache
{
  private static readonly DistributedCacheEntryOptions DefaultOptions =
    new DistributedCacheEntryOptions().SetAbsoluteExpiration(TimeSpan.FromDays(36500));
  ...
  {
    await _cache.SetStringAsync(key, value, DefaultOptions);
  }
  ...
  {
    if (!pattern.StartsWith('*')) pattern = _redisConfig.InstanceName + pattern;
  ...
  {
    await _cache.SetStringAsync(key, JSON.Serialize(value!), DefaultOptions);
  }

  public async Task SetAsync<T>(string key, T value, long minutes)
  {
    var options = new DistributedCacheEntryOptions().SetAbsoluteExpiration(TimeSpan.FromMinutes(minutes));
    await _cache.SetStringAsync(key, JSON.Serialize(value!), options);
  }
}
