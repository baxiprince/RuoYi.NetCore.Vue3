...
public static class ObjectExtensions
{
  ...
  public static string? ToUpperCamelCase2(this string str)
  {
    ...
    foreach (var s in strs) sb.Append(s.ToUpperCamelCase());
    return sb.ToString();
  }
  ...
  public static string ToUnderScoreCase(this string str)
  {
    ...
    for (var i = 0; i < chars.Length; i++)
    {
      ...
      if (i < str.Length - 1) nexteCharIsUpperCase = char.IsUpper(chars[i + 1]);
  ...
  public static string? SubstringBetween(this string? str, string open, string close)
  {
    ...
    if (start != -1)
    {
      ...
      if (end != -1) return str.Substring(start + open.Length, end - (start + open.Length));
    }
  ...
  {
    if (string.IsNullOrEmpty(str) || separator == null) return str;
  ...
  public static string ToDictString(this IDictionary dic)
  {
    var results = new List<string>();
    foreach (var key in dic.Keys) results.Add($"{key}:{dic[key]}");
  ...
  public static IDictionary<string, object> ToDictionary(this object input)
  {
    ...
    if (input is JObject jobj)
    {
      ...
      foreach (var (key, value) in jobj) dic.Add(key, value);
    ...
    if (input.GetType().HasImplementedRawGeneric(typeof(IDictionary<,>)))
    {
      ...
      foreach (var key in dicInput.Keys) dic.Add(key.ToString(), dicInput[key]);
  ...
  internal static object ToObject(this JsonElement jsonElement)
  {
    switch (jsonElement.ValueKind)
    {
      ...
      case JsonValueKind.Object:
        ...
        var dic = new Dictionary<string, object>();
        foreach (var item in enumerateObject) dic.Add(item.Name, item.Value.ToObject());
        return dic;

      case JsonValueKind.Array:
        var enumerateArray = jsonElement.EnumerateArray();
        var list = new List<object>();
        foreach (var item in enumerateArray) list.Add(item.ToObject());
        return list;
  ...
  {
    foreach (var (key, value) in newDic) dic.AddOrUpdate(key, value, (key, old) => value);
  }
