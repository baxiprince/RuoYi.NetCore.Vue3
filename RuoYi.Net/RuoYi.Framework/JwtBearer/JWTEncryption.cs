...
namespace RuoYi.Framework.JwtBearer;
...
public class JWTEncryption
{
    ...
    long? expiredTime = null, int refreshTokenExpiredTime = 43200, string tokenPrefix = "Bearer ", long clockSkew = 5)
  {
    ...
    {
      // 禁止使用刷新 Token 进行单独校验
      if (_refreshTokenClaims.All(k => context.User.Claims.Any(c => c.Type == k))) return false;
  ...
  {
    var tokenHandler = new JsonWebTokenHandler();
    if (tokenHandler.CanReadToken(accessToken)) return tokenHandler.ReadJsonWebToken(accessToken);
  ...
  internal static JWTSettingsOptions SetDefaultJwtSettings(JWTSettingsOptions options)
  {
    ...
    if (options.ValidateIssuerSigningKey == true) options.IssuerSigningKey ??= "$!%wi3n@z@w8Z!zG81#X%F^m1bA2syr#@5@Y";
    options.ValidateIssuer ??= true;
    if (options.ValidateIssuer == true) options.ValidIssuer ??= "RuoYi.Net";
    options.ValidateAudience ??= true;
    if (options.ValidateAudience == true) options.ValidAudience ??= "powerby RuoYi.Net";
  ...
  internal static Assembly GetFrameworkContext(Assembly callAssembly)
  {
    ...
    var furionAssemblyName = callAssembly.GetReferencedAssemblies()
                               .FirstOrDefault(u => u.Name == "RuoYi.Framework")
                             ?? throw new InvalidOperationException(
                               "No `RuoYi.Framework` assembly installed in the current project was detected.");

    // 加载 RuoYi 程序集
    var furionAssembly = AssemblyLoadContext.Default.LoadFromAssemblyName(furionAssemblyName);

    // 获取 App 静态类
    FrameworkApp = furionAssembly.GetType("RuoYi.Framework.App");
