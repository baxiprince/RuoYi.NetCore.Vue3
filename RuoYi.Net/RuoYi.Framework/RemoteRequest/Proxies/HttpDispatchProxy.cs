...
public class HttpDispatchProxy : AspectDispatchProxy, IDispatchProxy
{
  ...
  private HttpRequestPart BuildHttpRequestPart(MethodInfo method, object[] args)
  {
    ...
    if (retryPolicyAttribute != null)
      httpRequestPart.SetRetryPolicy(retryPolicyAttribute.NumRetries, retryPolicyAttribute.RetryTimeout);
  ...
  private static void SetBody(IEnumerable<MethodParameterInfo> parameters, HttpRequestPart httpRequestPart)
  {
    ...
    var filesParameters = parameters.Where(u => u.Parameter.ParameterType == typeof(HttpFile)
                                                ...
                                                || (u.Parameter.ParameterType.IsGenericType &&
                                                    typeof(IEnumerable).IsAssignableFrom(u.Parameter.ParameterType) &&
                                                    u.Parameter.ParameterType.GenericTypeArguments[0] == typeof(HttpFile)));
  ...
  private static void SetValidation(IEnumerable<MethodParameterInfo> parameters)
  {
    ...
    var validateParameters = parameters.Where(u =>
      u.Parameter.IsDefined(typeof(ValidationAttribute), true) && !u.Parameter.IsDefined(typeof(BodyAttribute), true));
  ...
  /// <param name="parameters"></param>
  /// <param name="httpRequestPart"></param>
  private static void SetJsonSerialization(MethodInfo method, IEnumerable<MethodParameterInfo> parameters,
    HttpRequestPart httpRequestPart)
  {
    // 判断方法是否自定义序列化选项
    var jsonSerializerOptions =
      parameters.FirstOrDefault(u => u.Parameter.IsDefined(typeof(JsonSerializerOptionsAttribute), true))?.Value
    ...
    foreach (var method in interceptorMethods)
    {
      ...
      switch (interceptor.Type)
      {
        ...
        case InterceptorTypes.Request:
          var onRequesting =
            (Action<HttpClient, HttpRequestMessage>)Delegate.CreateDelegate(typeof(Action<HttpClient, HttpRequestMessage>),
              method);
        ...
        case InterceptorTypes.Response:
          var onResponsing =
            (Action<HttpClient, HttpResponseMessage>)Delegate.CreateDelegate(typeof(Action<HttpClient, HttpResponseMessage>),
              method);
        ...
        case InterceptorTypes.Exception:
          var onException =
            (Action<HttpClient, HttpResponseMessage, string>)Delegate.CreateDelegate(
              typeof(Action<HttpClient, HttpResponseMessage, string>), method);
  ...
  private static void CallMethodInterceptors(IEnumerable<MethodParameterInfo> parameters, HttpRequestPart httpRequestPart)
  {
    ...
    foreach (var item in Interceptors)
    {
      ...
      switch (interceptor.Type)
      {
        // Client 创建拦截
        case InterceptorTypes.Initiate:
          if (item.Value is Func<HttpClient> clientProvider) httpRequestPart.SetClient(clientProvider);
          break;
        // 加载 Client 配置拦截
        case InterceptorTypes.Client:
          if (item.Value is Action<HttpClient> onClientCreating) httpRequestPart.OnClientCreating(onClientCreating);
          break;
        // 加载请求拦截
        case InterceptorTypes.Request:
          if (item.Value is Action<HttpClient, HttpRequestMessage> onRequesting) httpRequestPart.OnRequesting(onRequesting);
          break;
        // 加载响应拦截
        case InterceptorTypes.Response:
          if (item.Value is Action<HttpClient, HttpResponseMessage> onResponsing) httpRequestPart.OnResponsing(onResponsing);
          break;
        // 加载异常拦截
        case InterceptorTypes.Exception:
          if (item.Value is Action<HttpClient, HttpResponseMessage, string> onException) httpRequestPart.OnException(onException);
          break;
      }
    }
  }
