...
public static class OptionsBuilderExtensions
{
    ...
    where TOptions : class
  {
    ...
    if (!builderInterfaces.Any()) return optionsBuilder;

    // 逐条调用 .NET 底层选项配置方法
    foreach (var builderInterface in builderInterfaces) InvokeMapMethod(optionsBuilder, optionsBuilderType, builderInterface);
  ...
  {
    // 处理空对象或空值
    if (optionsBuilderTypes.IsEmpty()) throw new ArgumentNullException(nameof(optionsBuilderTypes));
    ...
    where TOptions : class
  {
    ...
    // 注册验证特性支持
    if (optionsBuilderAttribute?.ValidateDataAnnotations == true) optionsBuilder.ValidateDataAnnotations();
    ...
    , Type builderInterface)
  {
    ...
    var matchMethod = optionsBuilderType.GetMethods(bindingAttr)
      .First(u => u.Name == methodName ||
                  (u.Name.EndsWith("." + methodName) && u.GetParameters().Length == genericArguments.Length));
    ...
    , out object[] args)
  {
    ...
    if (isValidateMethod)
    {
      ...
      if (!string.IsNullOrWhiteSpace(arg1)) arg1Expression = Expression.Parameter(typeof(string), "arg1");
    }
