...
internal static class InternalApp
{
  ...
  internal static void AddJsonFiles(IConfigurationBuilder configurationBuilder, IHostEnvironment hostEnvironment)
  {
    ...
    // 处理控制台应用程序
    var _excludeJsonPrefixs =
      hostEnvironment == default ? excludeJsonPrefixs.Where(u => !u.Equals("appsettings")) : excludeJsonPrefixs;
    ...
    foreach (var group in jsonFilesGroups)
    {
      ...
      // 循环加载
      foreach (var jsonFile in files) configurationBuilder.AddJsonFile(jsonFile, true, true);
    }
  }
