...
public static class DependencyInjectionServiceCollectionExtensions
{
  ...
  static DependencyInjectionServiceCollectionExtensions()
  {
    ...
    GlobalServiceProxyType = App.EffectiveTypes
      .FirstOrDefault(u =>
        typeof(AspectDispatchProxy).IsAssignableFrom(u) && typeof(IGlobalDispatchProxy).IsAssignableFrom(u) && u.IsClass &&
        !u.IsInterface && !u.IsAbstract);
  ...
  private static IServiceCollection AddInnerDependencyInjection(this IServiceCollection services)
  {
    ...
    {
      // 获取注册方式
      var injectionAttribute = !type.IsDefined(typeof(InjectionAttribute))
        ? new InjectionAttribute()
        : type.GetCustomAttribute<InjectionAttribute>();
  ...
  private static void RegisterService(IServiceCollection services, Type dependencyType, Type type,
    InjectionAttribute injectionAttribute, IEnumerable<Type> canInjectInterfaces)
  {
    // 注册自己
  ...
  private static void Register(IServiceCollection services, Type dependencyType, Type type, InjectionAttribute injectionAttribute,
    Type inter = null)
  {
    // 修复泛型注册类型
    ...
    switch (injectionAttribute.Action)
    {
      case InjectionActions.Add:
        ...
        else
        {
          ...
          AddDispatchProxy(services, dependencyType, fixedType, injectionAttribute.Proxy, fixedInter);
        }
  ...
  private static void AddDispatchProxy(IServiceCollection services, Type dependencyType, Type type, Type proxyType, Type inter,
    bool hasTarget = true)
  {
    ...
    services.Add(ServiceDescriptor.Describe(inter, provider =>
    {
      ...
      if (hasTarget) proxy.Target = provider.GetService(type);
  ...
  private static void RegisterExternalServices(IServiceCollection services)
  {
    ...
    if (externalServices is { Definitions: not null })
    {
      ...
      foreach (var externalService in extServices)
      {
        ...
        // 加载代理拦截
        if (!string.IsNullOrWhiteSpace(externalService.Proxy))
          injectionAttribute.Proxy = Reflect.GetStringType(externalService.Proxy);
  ...
  private static ServiceLifetime TryGetServiceLifetime(Type dependencyType)
  {
    ...
    if (dependencyType == typeof(IScoped))
      return ServiceLifetime.Scoped;
    if (dependencyType == typeof(ISingleton))
