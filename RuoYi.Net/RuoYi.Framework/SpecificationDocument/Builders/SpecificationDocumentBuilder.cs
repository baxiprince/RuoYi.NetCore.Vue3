...
public static class SpecificationDocumentBuilder
{
  ...
  public static bool CheckApiDescriptionInCurrentGroup(string currentGroup, ApiDescription apiDescription)
  {
    ...
    if (typeof(Controller).IsAssignableFrom(method.DeclaringType) &&
        apiDescription.ActionDescriptor.ActionConstraints == null) return false;
    ...
    if (currentGroup == AllGroupsKey) return true;
  ...
  public static List<SpecificationOpenApiInfo> GetOpenApiGroups()
  {
    ...
    foreach (var group in DocumentGroups) openApiGroups.Add(GetGroupOpenApiInfo(group));
  ...
  internal static void BuildGen(SwaggerGenOptions swaggerGenOptions, Action<SwaggerGenOptions> configure = null)
  {
    ...
    // 支持控制器排序操作
    if (_specificationDocumentSettings.EnableTagsOrderDocumentFilter == true)
      ...
      if (File.Exists(assemblyXmlPath))
      {
        ...
        foreach (var memberElement in memberElementList)
        {
          ...
          if (value == null)
          {
            ...
            // 处理隐式实现接口的注释
            // 注释格式：M:RuoYi.Application.TestInheritdoc#Application#ITestInheritdoc#Abc(System.String)
            // 匹配格式：[A-Z]:[a-zA-Z_@\.]+\.
  ...
  {
    // 判断是否启用了授权
    if (_specificationDocumentSettings.EnableAuthorized != true ||
        _specificationDocumentSettings.SecurityDefinitions.Length == 0) return;
    ...
    // 添加安全需求
    if (openApiSecurityRequirement.Count > 0) swaggerGenOptions.AddSecurityRequirement(openApiSecurityRequirement);
  }
  ...
  private static void CustomizeIndex(SwaggerUIOptions swaggerUIOptions)
  {
    ...
    // 判断是否启用 MiniProfile
    var customIndex =
      $"{thisType.Namespace}.Assets.{(_appSettings.InjectMiniProfiler != true ? "index" : "index-mini-profiler")}.html";
    swaggerUIOptions.IndexStream = () =>
    {
      ...
      // 替换模板参数
      foreach (var (template, value) in indexArguments) htmlBuilder.Replace(template, value);
  ...
  private static IEnumerable<string> ReadGroups()
  {
    ...
    if (!controllers.Any())
    {
      ...
      // 启用总分组功能
      if (_specificationDocumentSettings.EnableAllGroups == true) defaultGroups.Add(AllGroupsKey);
    ...
    // 启用总分组功能
    if (_specificationDocumentSettings.EnableAllGroups == true) groups = groups.Concat(new[] { AllGroupsKey });
  ...
  public static IEnumerable<GroupExtraInfo> GetControllerGroups(Type type)
  {
    ...
    static IEnumerable<GroupExtraInfo> Function(Type type)
    {
      ...
      foreach (var group in apiDescriptionSettings.Groups) groupExtras.Add(ResolveGroupExtraInfo(group));
