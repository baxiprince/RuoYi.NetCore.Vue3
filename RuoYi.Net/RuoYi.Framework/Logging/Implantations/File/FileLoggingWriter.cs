...
internal class FileLoggingWriter
{
  ...
  /// </summary>
  /// <remarks>
  ///   只有配置了 <see cref="FileLoggerOptions.FileSizeLimitBytes" /> 或 <see cref="FileLoggerOptions.FileNameRule" /> 或
  ///   <see cref="FileLoggerOptions.MaxRollingFiles" /> 有效
  /// </remarks>
  ...
  private string GetNextFileName()
  {
    ...
    if (suffix.Length > 0 && int.TryParse(suffix, out var parsedIndex)) currentFileIndex = parsedIndex;

    // 【递增】部分 +1
    var nextFileIndex = currentFileIndex + 1;

    // 如果配置了最大【递增】数，则超出自动从头开始（覆盖写入）
    if (_options.MaxRollingFiles > 0) nextFileIndex %= _options.MaxRollingFiles;
  ...
  private void CheckForNewLogFile()
  {
    ...
    {
      return _options.FileSizeLimitBytes > 0
