...
public sealed partial class StringLoggingPart
{
  ...
  public void Log()
  {
    ...
    // 如果存在异常且事件 Id 不为空
    else if (Exception != null && EventId != null) logger.Log(Level, EventId.Value, Exception, Message, Args);

    // 释放临时日志工厂
    if (hasException) loggerFactory?.Dispose();
  }
  ...
  /// </summary>
  /// <returns></returns>
  internal (ILogger logger, ILoggerFactory? loggerFactory, bool hasException) GetLogger()
  {
    // 解析日志分类名
  ...
  private static ILoggerFactory CreateDisposeLoggerFactory()
  {
    ...
    Action<ConsoleFormatterExtendOptions> configure = opt =>
    {
