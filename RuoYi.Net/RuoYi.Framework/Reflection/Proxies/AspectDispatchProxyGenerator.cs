...
internal static class AspectDispatchProxyGenerator
{
  ...
  private static readonly MethodInfo s_dispatchProxyInvokeMethod =
    typeof(AspectDispatchProxy).GetTypeInfo().GetDeclaredMethod("Invoke");

  private static readonly MethodInfo s_dispatchProxyInvokeAsyncMethod =
    typeof(AspectDispatchProxy).GetTypeInfo().GetDeclaredMethod("InvokeAsync");

  private static readonly MethodInfo s_dispatchProxyInvokeAsyncTMethod =
    typeof(AspectDispatchProxy).GetTypeInfo().GetDeclaredMethod("InvokeAsyncT");
  ...
  private class ProxyBuilder
  {
    ...
    private void Complete()
    {
      ...
      for (var i = 0; i < args.Length; i++) args[i] = _fields[i].FieldType;
    ...
    private MethodBuilder AddMethodImpl(MethodInfo mi)
    {
      ...
      if (mi.ContainsGenericParameters)
      {
        ...
        for (var i = 0; i < ts.Length; i++) ss[i] = ts[i].Name;
      ...
      if (mi.ReturnType == typeof(Task)) invokeMethod = s_delegateInvokeAsync;
    ...
    private sealed class MethodInfoEqualityComparer : EqualityComparer<MethodInfo>
    {
      ...
      public override int GetHashCode(MethodInfo obj)
      {
        ...
        foreach (var parameter in obj.GetParameters()) hashCode ^= parameter.ParameterType.GetHashCode();
