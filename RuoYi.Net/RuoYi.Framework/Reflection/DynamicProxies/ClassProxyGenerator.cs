...
@constructor_methods
  ...
  static ClassProxyGenerator()
  {
    ...
    if (classType.IsSealed || classType.IsAbstract)
      throw new InvalidOperationException($"Type {classType} cannot be an abstract or sealed class.");
    ...
    // [代理]类型名称（含泛型）
    var genericDefinition = !classType.IsGenericType
      ? string.Empty
      : $"<{string.Join(',', classType.GetGenericTypeDefinition().GetGenericArguments().Select(t => t.Name))}>";
  ...
  ///   <see cref="Assembly" />
  /// </returns>
  private static Assembly CompileCSharpClassCode(string csharpCode, string assemblyName = default,
