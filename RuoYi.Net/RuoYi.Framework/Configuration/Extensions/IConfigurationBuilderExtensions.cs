...
using RuoYi.Framework.Configuration;
...
public static class IConfigurationBuilderExtensions
{
    ...
    , bool includeEnvironment = false)
  {
    ...
    {
      // 取得带环境文件名绝对路径
      var fileNameWithEnvironmentPath =
        ResolveRealAbsolutePath(fileNameWithEnvironmentPart.Replace("{env}", environment.EnvironmentName));
  ...
  {
    // 只有包含该参数才改变值
    if (parameters.ContainsKey(parameterName)) value = parameters[parameterName];
  }
}
