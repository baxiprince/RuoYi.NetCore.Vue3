...
public abstract class AbstractQuartzJob : IJob
{
  ...
  {
    var sysJob = context.MergedJobDataMap[ScheduleConstants.TASK_PROPERTIES] as SysJobDto;
    if (sysJob == null) return;

    try
    {
      Before(context, sysJob);
      if (sysJob != null) DoExecute(context, sysJob);
  ...
  private async Task After(IJobExecutionContext context, SysJobDto sysJob, Exception? e)
  {
    ...
    var runMs = now.ToUnixTimeMilliseconds() - startTimestamp;
    var errorMsg = e != null ? StringUtils.Substring(ExceptionUtils.GetErrorMessage(e), 0, 2000) : null;
