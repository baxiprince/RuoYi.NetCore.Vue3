...
public static class ScheduleUtils
{
  ...
  public static async Task CreateScheduleJob(IScheduler scheduler, SysJobDto job)
  {
    ...
    var jobDetail = JobBuilder.Create(jobType).WithIdentity(jobKey).Build();
    ...
    var trigger = TriggerBuilder.Create()
      ...
      .WithCronSchedule(job.CronExpression!, builder => { HandleCronScheduleMisfirePolicy(job, cronScheduleBuilder); }).Build();
    ...
    // 暂停任务
    if (job.Status!.Equals(ScheduleStatus.PAUSE.GetValue())) await scheduler.PauseJob(jobKey);
  }
  ...
  public static bool InWhiteList(string invokeTarget)
  {
    ...
    var packageName = StringUtils.SubstringBefore(invokeTarget, "(") ?? "";
    var count = StringUtils.CountMatches(packageName, ".");
    if (count > 1) return StringUtils.ContainsAnyIgnoreCase(invokeTarget, jobWhiteList);
    var classNamespace = AssemblyUtils.GetTaskAttributeClassNamespace(invokeTarget.Split(".")[0]);
  ...
  {
    var isConcurrent = "0".Equals(sysJob.Concurrent);
