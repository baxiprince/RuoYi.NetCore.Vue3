...
public class DataScopeAttribute : AbstractInterceptorAttribute
{
  ...
  public override async Task Invoke(AspectContext context, AspectDelegate next)
  {
    try
    {
      ...
      if (loginUser != null)
      {
        ...
        {
          var permission = StringUtils.DefaultIfEmpty(Permission, PermissionContextHolder.GetContext());
          DataScopeFilter(context, currentUser, DeptAlias, UserAlias, permission);
        }
      }
  ...
  private void DataScopeFilter(AspectContext context, SysUserDto user, string deptAlias, string userAlias, string permission)
  {
    ...
    foreach (var role in user.Roles ?? new List<SysRoleDto>())
    {
      var dataScope = role.DataScope ?? DataScope.Custom;
      if (dataScope != DataScope.Custom && conditions.Contains(dataScope)) continue;
      ...
      {
        sqlString.Append(
          $" OR {deptAlias}.dept_id IN ( SELECT dept_id FROM sys_dept WHERE dept_id = {user.DeptId} or find_in_set( {user.DeptId} , ancestors ) )");
      }
    ...
    if (conditions.IsEmpty()) sqlString.Append($" OR {deptAlias}.dept_id = 0 ");

    if (StringUtils.IsNotBlank(sqlString.ToString()))
    {
      var parameters = context.Parameters[0];
      if (parameters != null && parameters.GetType().BaseType != null && parameters.GetType().BaseType!.Equals(typeof(BaseDto)))
      {
        var baseEntity = (BaseDto)parameters;
