...
public class GenTableColumn : UserBaseEntity
{
  ...
  private static readonly List<string> _UsableColumns = new() { "ParentId", "OrderNum", "Remark" };
  ...
  private static readonly List<string> _SuperColumns = new()
    { "CreateBy", "CreateTime", "UpdateBy", "UpdateTime", "Remark", "ParentName", "ParentId", "OrderNum", "Ancestors" };
  ...
  {
    if (!string.IsNullOrEmpty(NetField)) return string.Concat(NetField.First().ToString().ToLower(), NetField.AsSpan(1));
