...
public class LogAttribute : Attribute, IAsyncActionFilter
{
  ...
  private async Task HandleLogAsync(MethodInfo actionMethod, IDictionary<string, object> parameterValues, FilterContext context,
    dynamic next)
  {
    // 排除 WebSocket 请求处理
  ...
  private string? GetOperParam(IDictionary<string, object> parameterValues, string[] excludeParamNames)
  {
    ...
    {
      if (excludeParamNames.Contains(parameterValue.Key)) continue;
