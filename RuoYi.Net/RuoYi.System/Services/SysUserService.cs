...
public class SysUserService : BaseService<SysUser, SysUserDto>, ITransient
{
  ...
  {
    var list = _sysRoleService.GetRolesByUserName(userName);
    if (!list.IsNotEmpty()) return string.Empty;
  ...
  {
    var list = _sysPostService.GetPostsByUserName(userName);
    if (!list.IsNotEmpty()) return string.Empty;
  ...
  {
    var userDto = await GetByUsernameAsync(user.UserName!);
    if (userDto != null && userDto.UserId != user.UserId) return false;
    return true;
  }
  ...
  {
    var userDto = await GetByPhoneAsync(user.Phonenumber!);
    if (userDto != null && userDto.UserId != user.UserId) return false;
    return true;
  }
  ...
  {
    var userDto = await GetByEmailAsync(user.Email!);
    if (userDto != null && userDto.UserId != user.UserId) return false;
    return true;
  }
  ...
  {
    if (user.UserId > 0 && SecurityUtils.IsAdmin(user.UserId)) throw new ServiceException("不允许操作超级管理员用户");
  }
  ...
  public async Task CheckUserDataScope(long? userId)
  {
    ...
    {
      var dto = new SysUserDto { UserId = userId };
      var users = await GetUserListAsync(dto);
      if (users.IsEmpty()) throw new ServiceException("没有权限访问用户数据！");
    }
  }
  ...
  public virtual bool InsertUser(SysUserDto user)
  {
    ...
    var succees = _sysUserRepository.Insert(user);
    // 新增用户岗位关联
    InsertUserPost(user);
    // 新增用户与角色管理
    InsertUserRole(user);
  ...
  {
    InsertUserRole(user.UserId ?? 0, user.RoleIds);
  }
  ...
  public void InsertUserPost(SysUserDto user)
  {
    ...
    {
      // 新增用户与岗位管理
      var list = new List<SysUserPost>();
      foreach (var postId in user.PostIds)
      {
        var up = new SysUserPost();
  ...
  {
    if (dtos.IsEmpty()) throw new ServiceException("导入用户数据不能为空！");
    var successNum = 0;
    var failureNum = 0;
    var successMsg = new StringBuilder();
    var failureMsg = new StringBuilder();
    var password = _sysConfigService.SelectConfigByKey("sys.user.initPassword");
    foreach (var user in dtos)
    {
      ...
      {
        // 验证是否存在这个用户
        var u = await _sysUserRepository.GetUserDtoByUserNameAsync(user.UserName!);
        if (u == null)
        {
          user.Validate();
      ...
      {
        failureNum++;
        var msg = "<br/>" + failureNum + "、账号 " + user.UserName + " 导入失败：";
