...
public class SysUserOnlineService : ITransient
{
  ...
  {
    if (StringUtils.Equals(ipaddr, user.IpAddr)) return LoginUserToUserOnline(user);
    return null;
  }
  ...
  {
    if (StringUtils.Equals(userName, user.UserName)) return LoginUserToUserOnline(user);
    return null;
  }
